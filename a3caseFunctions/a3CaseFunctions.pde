...
void setup() {
  ...
  for (int i = 0; i < calgaryTables.length; i++) {
   calgaryTables[i] = loadTable("calgary_abtest/daily_calgary_ab_" + i + ".csv");
   println(calgaryTables[i]);
  }
  ...
  daily_calgary_ab_2014 = loadTable("calgary_ab/daily_calgary_ab_2014.csv");

  savedX = new int[53];
  savedValues = new int[53];
  //getValues(daily_calgary_ab_2014);
  getValues(calgaryTables[2]);
} // End of setup()
